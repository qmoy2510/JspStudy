create table goods_tbl_003(
	...
	--입고일자
	in_date date
)select goods_cd,goods_nm,goods_price,cost,to_char(in_date,'yyyy-mm-dd') from goods_tbl_003 

update goods_tbl_003 set goods_cd = ?, goods_nm=?, goods_price=?,cost =?,in_date=?
...
select store_nm, sum(case when pay_type = '01' then goods_price*sale_cnt else 0 end) , sum(case when pay_type = '02' then goods_price*sale_cnt else 0 end), (sum(case when pay_type = '01' then goods_price*sale_cnt else 0 end)+ sum(case when pay_type = '02' then goods_price*sale_cnt else 0 end))   from goods_tbl_003 g, store_tbl_003 s, sale_tbl_003 ss  where s.store_cd = ss.store_cd and g.goods_cd = ss.goods_cd group by store_nm order by 4 desc

select case when sale_fg='1' then '판매' when sale_fg='2' then '판매취소' end, sale_no, sale_ymd, goods_nm, sale_cnt, (sale_cnt*goods_price), case when pay_type = '01' then '현금' when pay_type = '02' then '카드' end from goods_tbl_003 g, store_tbl_003 s, sale_tbl_003 ss where s.store_cd = ss.store_cd and g.goods_cd = ss.goods_cd and store_nm = '이태원점'
...
goods_cd,goods_nm,goods_price,cost,to_char(in_date,'yyyy-mm-dd') from  goods_tbl_003
create sequence numnum 


drop sequence text;
create SEQUENCE text start with 20;
